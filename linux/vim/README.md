...
- 정규표현식(**패턴** 부분에 사용)을 이용해 치환 가능하다: _g/패턴/범위/문자열1/문자열2/라인범위
> 정규표현식
>? 
...
| g/**\\(^[^#].\*TEST\\\)**/범위/문자열1/문자열2/라인범위 | 범위에 대해  _#문자로 시작하지 않는 TEST문자열_ 이 있는 <br>모든or라인당 하나의(라인 범위에 따라 상이) 문자열 1을 문자열 2로 수정 |
