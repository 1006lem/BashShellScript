...
| 명령어 | 설명 |
| :---: | --- |
| **i** | 현재 커서 위치에서 문자 삽입( **insert** ) |
| **a** | 현재 커서 위치의 다음 칸부터 문자 삽입 ( **append** ) |
...
- **명령 모드** 로 진입하여 사용
- _:(줄 수1),(줄 수2)d_ 는 줄 수 1~ 줄 수 2 행 삭제 <br>
- **[횟수] d 대상** 또는 **d [횟수] 대상** 꼴을 가진다<br>
  - **(1) 횟수**는 기본 1 <br>
...
| 명령어 | 설명 |
| :---: | --- |
| **x**, dI | - 커서 위치의 **문자 삭제**<br>- x의 개수만큼 문자 삭제 _(xxx은 문자 3개 삭제)_ |
| **dd** | 커서가 있는 **행 전체 삭제** |
...
| s/문자열1/문자열2/g | EX모드 | 현재 커서가 있는 행의 모든 문자열1을 문자열 2로 대체 |
| s/문자열1/문자열2| EX모드 | 현재 커서가 있는 행의 첫번째 문자열1을 문자열 2로 대체 |
| 1,3s/문자열1/문자열2/g | EX모드 | 1~3행에서 문자열 1을 문자열 2로 대체 |
| -1,+3s/문자열1/문자열2/g | EX모드 | 현재 커서에서 위로 1개, 아래로 3개 행에서<br> 문자열 1을 문자열 2로 대체(현재 커서 행도 포함) |
...
### -추가 EX 모드 명령어
- !command를 이용해 외부 명령 실행 가능 (!dls 또는 !rm 파일이름)

| 명령어 | 설명 |
| :---: | --- |
| set nu | 작성한 파일의 라인 번호 출력 |
| set nonu | 작성한 파일의 라인 번호 출력 취소 |
| !dir | 디렉토리 리스트 불러옴(기존 파일명 출력) |
| help | 도움말 제공 |
| help 명령어 | 명령어에 대한 도움말 |
...
- .vimrc파일을 직접 수정하기도 하지만, jellybeans와 같은 colorscheme을 적용하기도 한다
