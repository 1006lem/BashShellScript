...
- 파일 시스템(File System)이란 컴퓨터에서 파일이나 자료를 쉽게 접근할 수 있도록 보관하는 체제를 말한다.
~~- 리눅스 파일 시스템 정보는 man fs명령어를 통해서도 얻을 수 있다.~~
- 리눅스에서는 다음과 같은 파일 시스템을 지원한다. 
#### - 1. minix
- 리눅스에서 가장 먼저 사용된 파일 시스템
- 파일 이름은 최대 30문자, 파일 시스템당 최대 64MB 지원 등 성능 제한이 있다
#### - 2. ext2
- 파일 이름은 256 바이트, 단일 파일당 2GB, 디스크 4TB를 지원한다.
