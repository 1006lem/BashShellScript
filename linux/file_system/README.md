...
| 명령어 | 특징 |
| :----: | ------------------------------------------- |
| super block | 시스템 정보 표시 |
| inode block | - inode 기반 파일의 메타데이터를 저장한 블록<br> -파일의 소유권, 허가권, 파일 종류, 생성 시간, 데이터 저장 위치 등이 있다 <br> -`ls -al` 명령어로 확인할 수 있는 정보를 담는다 |
| data block | - inode block 안에 저장<br> - 파일 크기에 따라 Direct block, Single Indirect block, Double indirect block, Triple indirect block으로 나눈다. |
