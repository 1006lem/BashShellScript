...
-[1. 리눅스 파일 시스템이란?](#1-리눅스-파일-시스템이란)
  - [1. inode](#1-inode)
  - [2. 리눅스의 대표적인 파일 시스템](#2-리눅스의-대표적인-파일-시스템)
    - [1. MINIX](#1-minix)
    - [2. EXT2](#2-ext2)
    - [3. xiafs](#3-xiafs)
    - [4. XFS](#4-xfs)

-[2. 리눅스 파일 종류](#2-리눅스-파일-종류)
  -[1. 일반 파일](#1-일반-파일)
  -[2. 디렉토리 파일](#2-디렉토리-파일)
  -[3. 링크 파일](#3-링크-파일)
    - [1. 하드 링크 파일](#1-하드-링크-파일)
    - [2. 소프트 링크(심볼릭 링크) 파일](#2-소프트 링크심볼릭 링크-파일)
  -[4. 장치 파일](#4-장치-파일)
...
- 윈도우에서의 바로가기 기능과 유사
#### - 1. 하드 링크 파일
- 원본 파일의 inode를 그대로 참조하는 파일<br>
- 디렉토리는 링크할 수 없다
- 원본 파일이 지워져도 문제되지 않는다
#### - 2. 소프트 링크(심볼릭 링크) 파일
...
<!-- https://velog.io/@yaaloo/OS-%ED%8C%8C%EC%9D%BC-%EC%8B%9C%EC%8A%A4%ED%85%9C-->
   ...
   *더 자세한 내용은 다음 링크의 **[2.4 할당 방법](https://github.com/boanlab/study-notes/blob/main/major_subjects/operating_system/06_fileSystem/README.md)** 을 참고하자*
   ...
   - 블록 체인: 블록을 불연속적으로 저장하여 linked list로 관리<br>
  linked list의 단점을 모두 갖는다(마지막 블록을 찾으려면 첫번째 블록부터 방문)
   - 인데스 블록 기법: 파일마다 색인 블록을 두고, <br>
     파일이 할당된 블록의 모든 포인터를 색인 블록에 포함시켜 접근하도록 하는 방법
     원하는 블록에 바로 접근이 가능하나, <br>인덱스 블록이 유실되었을때 파일의 전체 데이터가 소실된다는 단점이 있다.
  [↩](#a3)
